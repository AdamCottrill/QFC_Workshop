...
- open a command prompt or open emacs and type ~Alt-x shell ret~
...
- type ~<F8>~ to link and compile the simple model
- once the model is compiled, type ~<F9>~ to run the model
